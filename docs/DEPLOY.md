...
Si la cuenta `[AWS_ACCOUNT_ID_REDACTED]` todavía no tiene el provider de GitHub:
...
```json
{
  ...
  "Statement": [{
    "Effect": "Allow",
    "Principal": {
      "Federated": "arn:aws:iam::[AWS_ACCOUNT_ID_REDACTED]:oidc-provider/token.actions.githubusercontent.com"
    },
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:ecr:us-west-2:[AWS_ACCOUNT_ID_REDACTED]:repository/ngcourses-frontend",
        "arn:aws:ecr:us-west-2:[AWS_ACCOUNT_ID_REDACTED]:repository/ngcourses-backend"
      ]
    }
  ]
}
...
| Secret | Valor |
| --- | --- |
| `AWS_DEPLOY_ROLE_ARN` | `arn:aws:iam::[AWS_ACCOUNT_ID_REDACTED]:role/GitHubActionsDeployRole` |
...
SHA=abc1234...
docker pull [AWS_ECR_REGISTRY_REDACTED]/ngcourses-frontend:$SHA
docker tag [AWS_ECR_REGISTRY_REDACTED]/ngcourses-frontend:$SHA \
          [AWS_ECR_REGISTRY_REDACTED]/ngcourses-frontend:latest
